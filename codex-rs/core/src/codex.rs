...
async fn handle_container_exec_with_params(
    ...
    let sandbox_type = match safety {
        ...
        SafetyCheck::AskUser => {
            // Add Nova-style friendly security advice when asking for approval.
            let nova_reason = crate::security_advisor::compute_security_advice(&params.command);
            let combined_reason = match (params.justification.clone(), nova_reason) {
                (Some(j), Some(r)) => Some(format!("{j}\n\nNova: {r}")),
                (Some(j), None) => Some(j),
                (None, Some(r)) => Some(format!("Nova: {r}")),
                (None, None) => None,
            };
            let rx_approve = sess
