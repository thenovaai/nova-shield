...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        // Render project ASCII art in accent color (fallback to 256-color orange if truecolor is unavailable)
        let accent = {
        ...
        let lines = vec![
            Line::from("model changed:".fg(Color::Rgb(255, 165, 0)).bold()),
...
fn new_parsed_command(
    ...
    match output {
        None => {
            let mut spans = vec!["⚙︎ Working".fg(Color::Rgb(255, 165, 0)).bold()];
...
fn new_exec_command_generic(
    ...
    if let Some(first) = cmd_lines.next() {
        let mut spans: Vec<Span> = vec!["⚡ Running".fg(Color::Rgb(255, 165, 0))];
        if let Some(st) = start_time {
            let dur = exec_duration(st);
            spans.push(format!(" • {dur}").dim());
        }
...
pub(crate) fn new_diff_output(message: String) -> PlainHistoryCell {
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from("/diff".fg(Color::Rgb(255, 165, 0))));
...
pub(crate) fn new_status_output(
    ...
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from("/status".fg(Color::Rgb(255, 165, 0))));
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        Line::from("/mcp".fg(Color::Rgb(255, 165, 0))),
        Line::from(""),
...
pub(crate) fn new_stream_error_event(message: String) -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        vec!["⚠ ".fg(Color::Rgb(255, 165, 0)).bold(), message.dim()].into(),
        "".into(),
    ];
    PlainHistoryCell { lines }
}
...
pub(crate) fn new_patch_event(
    ...
    let title = match &event_type {
        ...
        PatchEventType::ApplyBegin {
            ...
            let lines: Vec<Line<'static>> = vec![
                Line::from("✏️ Applying patch".fg(Color::Rgb(255, 165, 0)).bold()),
                Line::from(""),
            ];
            return PlainHistoryCell { lines };
        }
...
pub(crate) fn new_patch_apply_failure(stderr: String) -> PlainHistoryCell {
    ...
    // Failure title
    lines.push(Line::from(
        "✘ Failed to apply patch".fg(Color::Rgb(255, 165, 0)).bold(),
    ));
...
pub(crate) fn new_patch_apply_success(stdout: String) -> PlainHistoryCell {
    ...
    // Success title
    lines.push(Line::from(
        "✓ Applied patch".fg(Color::Rgb(255, 165, 0)).bold(),
    ));
...
pub(crate) fn new_reasoning_block(
    ...
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from("thinking".fg(Color::Rgb(255, 165, 0)).italic()));
